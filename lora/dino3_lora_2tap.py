# LoRA + Upgraded Decoder experiment — 2-tap variant (blocks 10, 11 only).
# Identical to lora/dino3_lora.py except tap_blocks=[10, 11] instead of the
# default [2, 5, 8, 11].  Weight analysis (weight_analysis/dinov3_otherlayers)
# showed blocks 7 & 8 are near-redundant; this run tests whether two deep taps
# suffice (or even improve) over four.
#
...
#   !python /kaggle/working/cv_project_saliency/lora/dino3_lora_2tap.py
#
...
RESUME_FROM = None  # e.g. "/kaggle/input/lora-resume/resume_dinov3_lora_upgraded_2tap.pth"

# --- Only 2 taps: the two blocks that actually matter ---
TAP_BLOCKS = [10, 11]
...
# Append the cloned repository folder to Python's path
...
# --- Extractor: LoRA-adapted or frozen DINOv3 (2 taps) ---
if USE_LORA:
    extractor = LoraDinoV3ViT(tap_blocks=TAP_BLOCKS, grad_checkpointing=True).to(device)
