...
# NOTE (Kaggle): `timm` is preinstalled. Downloading the MAE weights from the
# HuggingFace hub needs internet enabled (Settings -> Internet: on).
...
for epoch in range(epochs):
    ...
    if val_loss < val_min:
        patience = 0
        val_min = val_loss
        torch.save(decoder.state_dict(), "/kaggle/working/best_mae_decoder.pth")
    ...
    if patience > 3:
        ...
        decoder.load_state_dict(torch.load("/kaggle/working/best_mae_decoder.pth"))
        break
...
print(f"Final Model Benchmark (MAE ViT-B Backbone) (hidden_dim=256):")
