...
# NOTE (Kaggle): `timm` is preinstalled. Downloading the ViT weights from
# HuggingFace needs internet enabled (Settings -> Internet: on).
...
# Swap -> ViT; everything downstream is backbone-agnostic.
extractor = ViT().to(device)

# Architecture Initialization. hidden_dim is set to 256.
decoder = Decoder(in_channels_list=extractor.out_channels, hidden_dim=512).to(device)
...
for epoch in range(epochs):
    ...
    if val_loss < val_min:
        patience = 0
        val_min = val_loss
        torch.save(decoder.state_dict(), "/kaggle/working/best_vit_decoder.pth")
    ...
    if patience > 3:
        ...
        decoder.load_state_dict(torch.load("/kaggle/working/best_vit_decoder.pth"))
        break
...
print(f"Final Model Benchmark (ViT-B Backbone) (hidden_dim=512):")
