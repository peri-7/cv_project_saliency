...
def train_one_epoch_online(extractor, decoder, dataloader, optimizer, criterion, device):
    """End-to-end training epoch: frozen extractor, trainable decoder."""
    extractor.eval()
    decoder.train()
    ...
    for images, target_map, _ in dataloader:
        ...
        # Run the frozen backbone without gradients.
        with torch.no_grad():
...
def evaluate_model_online(extractor, decoder, dataloader, criterion, device):
    """End-to-end validation loss. Both extractor and decoder frozen."""
    extractor.eval()
    decoder.eval()
...
def test_model_online(extractor, decoder, dataloader, criterion, device, baseline_prob=None):
    """End-to-end test pass: continuous losses plus discrete metrics."""
    extractor.eval()
    decoder.eval()
